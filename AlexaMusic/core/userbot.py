...
class Userbot(Client):
    ...
    async def start(self):
        LOGGER(__name__).info(f"بدء المساعدين ")
        if config.STRING1:
            await self.one.start()
            try:
                await self.one.join_chat('ql8ql')
            except:
                pass
            assistants.append(1)
            try:
                await self.one.send_message(
                    config.LOG_GROUP_ID,
                    "بدء المساعد الان حان الوقت بالاستمتاع بالموسيقى على تيليجرام.",
                )
            except:
                LOGGER(__name__).error(
                    f"فشل حساب المساعد في الوصول إلى مجموعة السجل.  تأكد من إضافة مساعدك إلى مجموعة السجل الخاصة بك وترقيتك كمسؤول!  " "
                )
                sys.exit()
            ...
            LOGGER(__name__).info(f"بدء المساعد بواسطة{self.one.name}")
        if config.STRING2:
            await self.two.start()
            try:
                await self.two.join_chat("ql8ql")
            except:
                pass
            assistants.append(2)
            try:
            ...
            LOGGER(__name__).info(f"بدء المساعد الثاني بواسطة{self.two.name}")
        if config.STRING3:
            await self.three.start()
            try:
                await self.three.join_chat("ql8ql")
            except:
                pass
            assistants.append(3)
            try:
            ...
            LOGGER(__name__).info(f"بدء المساعد الثالث بواسطة {self.three.name}")
        if config.STRING4:
            await self.four.start()
            try:
                await self.four.join_chat("ql8ql")
            except:
                pass
            assistants.append(4)
            try:
       ...
       "بدء المساعد الان حان الوقت بالاستمتاع بالموسيقى على تيليجرام",
            ...
            LOGGER(__name__).info(f"بدء المساعد الرابع بواسطة{self.four.name}")
        if config.STRING5:
            await self.five.start()
            try:
                await self.five.join_chat("ql8ql")
            except:
                pass
            assistants.append(5)
            try:
            ...
            LOGGER(__name__).info(f"بدء المساعد  الخامس بواسطة{self.five.name}")
